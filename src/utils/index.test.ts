...
import { getMimeType, sortByCreatedAt, truncate } from '.'
...
describe('#getMimeType', () => {
  test('拡張子からMIMEタイプを取得できること', () => {
    const result1 = getMimeType('.tmp/service/test.jpg')
    const result2 = getMimeType('audio.mp3')
    expect(result1).toBe('image/jpeg')
    expect(result2).toBe('audio/mpeg')
  })
  test('不明な拡張子の場合はapplication/octet-streamが返されること', () => {
    const result = getMimeType('unknown')
